...
class UserDAO {
  ...
  */
    ...
    try {
      return User.query()
        .insert({
          ...
          contraseña: raw(`AES_ENCRYPT('${password}','${process.env.ENCRYPTION_KEY}')`),
          nombre: name,
          rolUsuarioId: roleId,
        });
    } catch (error) {
  ...
  static async getAll(User) {
    try {
      return await User.query()
        .select('Usuario.id', 'Usuario.nombre', 'Usuario.nombre_usuario', 'RolUsuario.nombre AS rol')
  ...
  static async getById(User, id) {
    try {
      return await User.query()
        .select('Usuario.id', 'Usuario.nombre_usuario', 'Usuario.nombre', 'RolUsuario.nombre AS rol')
        .join('RolUsuario', 'Usuario.rol_usuario_id', 'RolUsuario.id')
        .findById(id)
        .where('eliminado', 0);
    } catch (error) {
  ...
  static async getByUsername(User, username) {
    try {
      const [user] = await User.query()
        .select('Usuario.id', 'Usuario.nombre', 'RolUsuario.nombre AS rol', raw(`AES_DECRYPT(contraseña, '${process.env.ENCRYPTION_KEY}') AS contraseña`))
        .join('RolUsuario', 'Usuario.rol_usuario_id', 'RolUsuario.id')
  ...
  static async update(User, { id, name, password }) {
    ...
    if (password) {
      queryObject.contraseña = raw(`AES_ENCRYPT('${password}','${process.env.ENCRYPTION_KEY}')`);
    }
    try {
