  ...
  async getLanguageCode({ language }) {
    try {
      if (language.length === 2) {
        // Buscar por código
        const exists = await LanguageDAO.doesExist(this.#modelInstance, { code: language });
        if (exists) {
          return language.toUpperCase();
        }
      } else {
        // Busca por nombre
        const result = await LanguageDAO.getByName(this.#modelInstance, { name: language, errCode: 'EDA10' });
